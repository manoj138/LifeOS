...
import { Volume2, VolumeX, Lock, Globe, Sparkles, Sliders, Radio, Mic } from 'lucide-react';
    ...
    <div className="relative flex items-center">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 bg-[#12121a]/90 hover:bg-[#181824] border border-white/15 hover:border-purple-500/50 rounded-full px-2.5 sm:px-3.5 py-1 sm:py-1.5 backdrop-blur-2xl shadow-[0_0_20px_rgba(168,85,247,0.25)] transition-all">
        ...
        >
          ...
          {/* Responsive Fixed-Width Label Box (Clean Non-Duplicated UI) */}
          <div className="w-24 sm:w-32 min-w-[96px] sm:min-w-[128px] max-w-[96px] sm:max-w-[128px] h-7 flex flex-col justify-center overflow-hidden">
            {isVoiceModalOpen ? (
              <div className="flex items-center gap-1.5 text-purple-300 font-mono">
                ...
                <span className="text-xs font-bold tracking-tight truncate">HUD Active</span>
              </div>
            ) : isSpeaking ? (
      ...
      {showSettingsPopover && (
        <div className="absolute top-12 right-0 z-50 w-64 bg-[#12121a]/95 border border-white/20 rounded-2xl p-3.5 shadow-2xl backdrop-blur-2xl space-y-2.5 text-xs">
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="text-[10px] text-gray-300 font-medium">Assistant Language:</span>
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={() => setLanguage('mr')}
                className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${
                  language === 'mr' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40' : 'text-gray-400 hover:text-white'
                }`}
              >
                मराठी
              </button>
              <button
                ...
                className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${
                  language === 'en' ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/40' : 'text-gray-400 hover:text-white'
                }`}
              >
                English
              </button>
          ...
          <div className="flex items-center justify-between pt-1 border-t border-white/10">
            <span className="text-[10px] text-gray-300 font-medium flex items-center gap-1">
              <Radio className={`w-3 h-3 ${isHandsFreeEnabled ? 'text-emerald-400 animate-pulse' : 'text-gray-500'}`} />
              Wake Word ("Hey LifeOS"):
            </span>
            <button
              type="button"
              onClick={() => setIsHandsFreeEnabled(!isHandsFreeEnabled)}
              className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${
                isHandsFreeEnabled ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-300 border border-rose-500/40'
              }`}
            >
